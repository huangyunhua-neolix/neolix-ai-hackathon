...
| 一批市民投诉，AI 归并分析定位车辆 | 市民投诉分析卡片（工单+报告） |
| 数据列表、筛选结果 | 表格数据卡片 |
...
适用：输入一批市民投诉（如 12345 热线转办），AI 语义聚类归并后定位车辆、分类输出诊断和回复话术。
流程：投诉工单卡片（grey）→ AI 归并分析 → 投诉分析报告卡片（purple），成对发送。
